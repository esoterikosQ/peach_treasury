...
### 단독 출금 SMS 파싱 누락 수정
- `[신한 슈퍼SOL]` 헤더 없이 오는 단독 출금 SMS가 파싱되지 않던 버그 수정
- 원인: 5/30 중복 방지 리팩토링(4551e3d) 시 `출금` + `80001` 단독 핸들러가 삭제됨
- 증상: `출금 1,400원 아이스무빙(강 110-***-*80001 2026.06.01 14:48:45` → `파싱 결과: None`
- 수정: `[신한 슈퍼SOL]` 핸들러 뒤에 단독 출금 핸들러 재추가 (체크카드 중복 방지 로직 포함)
